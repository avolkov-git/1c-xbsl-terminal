...
Обновлено 2026-09-22 по рабочему дереву: [разметка](src/webview.ts),
...
50–180px. Список и карандаш переименования составляют левую группу
шириной до 212px с промежутком 4px. Статус следует за ней. Очистка,
закрытие и настройки прижаты к правому краю через `margin-left: auto`;
кнопки этой группы не сжимаются. До ширины 360px статус скрывается,
промежуток групп сокращается до 4px. Проверены ширины 320, 361, 600 и
1100px, в том числе с длинным названием сессии.
...
- **Toolbar:** список, карандаш «Переименовать сессию», статус; справа
  «Очистить экран», «Закрыть сессию», «Путь к Bash».
  Создание сессии остаётся в шапке IDE и в пустом состоянии панели.
  Иконки имеют `aria-label` и `title`.
  Без сессии переименование, очистка и закрытие отключены (opacity 0.35). Наведение меняет
...
- **Выбор сессии:** нативный `select` с именем «Сессия Bash». Завершённые
  ...
  Полное название находится в `title`; длинный текст не расширяет toolbar.
  Карандаш следует за списком в порядке Tab и открывает штатное поле ввода IDE.
  Переименование сохраняет выбранную сессию, её процесс и вывод. Отмена
  оставляет прежнее имя. Имена живут до закрытия сессии.
